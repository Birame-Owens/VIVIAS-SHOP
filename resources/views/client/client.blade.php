...
<body class="antialiased">
    ...
    <div id="client-app">
        ...
        <div id="app-loader" style="min-height: 100vh; display: flex; align-items: center; justify-content: center; background: #fdfbf7;">
            <div style="text-align: center;">
                <div style="width: 60px; height: 60px; border: 4px solid #e5e5e5; border-top: 4px solid #1a1a1a; border-radius: 50%; animation: spin 1s linear infinite; margin: 0 auto 20px;"></div>
                <p id="loading-message" style="color: #666; font-size: 14px; text-transform: uppercase; letter-spacing: 2px;">Chargement...</p>
                <p id="loading-details" style="color: #999; font-size: 12px; margin-top: 10px;"></p>
            </div>
        </div>
    </div>
    
    {{-- Timeout detection pour mobile --}}
    <script>
        let loadStartTime = Date.now();
        let timeoutCheck = setInterval(function() {
            let elapsed = (Date.now() - loadStartTime) / 1000;
            let details = document.getElementById('loading-details');
            if (details) {
                details.textContent = Math.floor(elapsed) + 's';
            }
            
            if (elapsed > 10) {
                clearInterval(timeoutCheck);
                let message = document.getElementById('loading-message');
                if (message && !window.reactMounted) {
                    message.textContent = 'Erreur de chargement';
                    details.innerHTML = 'Connexion lente détectée.<br>Veuillez rafraîchir la page.';
                    details.style.color = '#ff0000';
                }
            }
        }, 500);
    </script>
    ...
    <a 
        href="https://wa.me/{{ config('app.whatsapp_number', '221784661412') }}" 
        target="_blank"
        rel="noopener noreferrer"
        class="fixed bottom-6 right-6 z-30 w-14 h-14 bg-green-500 rounded-full flex items-center justify-center shadow-lg hover:bg-green-600 transition-all hover:scale-110"
        style="text-decoration: none; min-width: 56px; min-height: 56px; max-width: 56px; max-height: 56px;"
        aria-label="Contacter sur WhatsApp"
    >
        <svg class="w-8 h-8 text-white" style="width: 32px; height: 32px; min-width: 32px; min-height: 32px;" fill="currentColor" viewBox="0 0 24 24">
